...
function MenuModuleItem({ data, children, activePath }: PropsWithChildren<MenuModuleItemProps>) {
  ...
  return (
    <MenuListItem className={classNames({ "active": isActive !== undefined ? isActive : false || activePath })}>
      {isExpandable
        ...
        : (<MenuListItemLink as={NavLink} to={data.path}><span>{data.name}</span></MenuListItemLink>)
      }
      {children}
    </MenuListItem>
  )
...
export function MenuModule({ data }: MenuModuleProps) {
  const currentMenuSelected = useMemo(() => extractPathFromList(window.location.pathname, data.menus, "path", "children").map(m => m.path), [data.menus, window.location.pathname]);

  const iterateMenu = useCallback((menu: Menu) => {
    const hasChildMenu = !!menu.children?.length;
    const isActivePath = currentMenuSelected.some(path => path === menu.path)
    ...
    return (
      ...
      >
        {hasChildMenu ? <SubMenuList>{menu.children?.map(iterateMenu)}</SubMenuList> : null}
      </MenuModuleItem>
    )
